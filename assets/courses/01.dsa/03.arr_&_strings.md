...
> 🧠 **Mental Model:** Arrays and strings are the **bread and butter** of coding interviews. Think of them as your training ground — 80% of interview problems reduce to clever array/string manipulation. Master these patterns and you'll crack most problems!
...
> 🧠 **Mental Model:** Think of frequency tables like **counting votes in an election**. You tally each candidate's votes in separate buckets. At the end, one quick scan tells you everything about the distribution!
...
> 🧠 **Mental Model:** Like **reading a book twice** — first pass to understand the plot, second pass to catch the foreshadowing. First scan gathers intel, second scan uses it!
...
> 🧠 **Mental Model:** Like **sorting laundry** — whites go left, colors go right. You use pointers as "boundaries" between categories and swap items to their correct side.
...
> 🧠 **Mental Model:** Think of strings as **trains of characters**. Each operation is like coupling/decoupling train cars. Be careful — creating new strings is like building new trains from scratch (expensive!), but modifying in-place is just rearranging existing cars.
...
```
What kind of array/string problem is this?
│
├─ Counting occurrences / finding duplicates?
│   ├─ Elements in small range [0, k]? ──► Array-based frequency table O(k)
│   ├─ Elements can be anything? ────────► unordered_map O(n)
│   └─ Need sorted unique elements? ─────► map O(n log n)
│
├─ Need to compute something for each position using neighbors?
│   ├─ Can info be accumulated? ─────────► Prefix/Suffix arrays
│   └─ Need both directions? ────────────► Two scans pattern
│
├─ Need to rearrange elements?
│   ├─ Two categories? ──────────────────► Two-pointer partition
│   ├─ Three categories? ────────────────► Dutch National Flag (3 pointers)
│   └─ Keep relative order? ─────────────► Stable partition
│
...
│   ├─ Variable size, shrinkable? ───────► Variable sliding window
│   └─ Need all elements once? ──────────► Hash set + sliding window
│
└─ String manipulation?
    ├─ Many concatenations? ─────────────► Reserve space or use vector<char>
    ├─ Character frequency? ─────────────► int freq[26] or freq[256]
    └─ Pattern matching? ────────────────► See string algorithms chapter
```

---

## 🐞 Debug Corner: Array/String Pitfalls

### Bug #1: Off-by-One in Two Scans

**❌ Buggy Implementation:**
```cpp
// Product except self - buggy version
vector<int> productExceptSelf(vector<int>& arr) {
    int n = arr.size();
    vector<int> result(n, 1);
    
    int prefix = 1;
    for (int i = 0; i <= n; i++) {  // BUG: i <= n goes out of bounds!
        result[i] = prefix;
        prefix *= arr[i];
    }
    // ...
...
**🔍 The Bug:** Loop goes to `i <= n` instead of `i < n`, causing array out-of-bounds access.
...
### Bug #2: String Concatenation in Loop
...
string buildString(int n) {
    string s = "";
    for (int i = 0; i < n; i++) {
        s = s + "a";  // BUG: Creates new string each iteration!
    }
    return s;
}
// Time: O(n²) because string copy happens each time!
```

**🔍 The Bug:** Using `s = s + "a"` creates a new string object each iteration, copying all existing characters.
...
string buildString(int n) {
    ...
    s.reserve(n);  // Pre-allocate
    for (int i = 0; i < n; i++) {
        s += "a";  // Appends to existing string
    }
    return s;
}
// Time: O(n)
...
### Bug #3: Dutch Flag - Not Moving Mid Pointer
...
void sortColors(vector<int>& arr) {
    ...
    while (mid <= high) {
        if (arr[mid] == 0) {
            ...
            swap(arr[mid++], arr[high--]);  // BUG: mid++ here!
        }
    }
}
```

**🔍 The Bug:** When swapping with `high`, we increment `mid`. But the element swapped FROM high hasn't been examined yet!

**✅ Correct Implementation:**
```cpp
if (arr[mid] == 2) {
    swap(arr[mid], arr[high--]);  // DON'T increment mid!
}
// mid stays same, will examine the newly swapped element next iteration
```

---

### Bug #4: Frequency Array with Negative Characters
...
bool hasAllUnique(string s) {
    int freq[128] = {0};
    for (char c : s) {
        freq[c]++;  // BUG: If c is negative (signed char), index is negative!
    }
}
```

**🔍 The Bug:** `char` is signed by default on some systems. Characters > 127 become negative indices!
...
bool hasAllUnique(string s) {
    ...
    // OR
    for (char c : s) {
        freq[(unsigned char)c]++;  // Cast to unsigned
    }
}
```
