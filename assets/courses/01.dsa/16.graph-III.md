...
> 🧠 **Mental Model:** These algorithms find **structural vulnerabilities** in networks. Bridges = the one highway whose closure splits a city in two. Articulation points = keystone people whose departure breaks teams apart. SCCs = friend groups where everyone can reach everyone!

---

## Table of Contents
1. Critical Connections Overview
2. Bridges (Critical Edges)
3. Articulation Points (Cut Vertices)
4. Strongly Connected Components (SCC)
5. Kosaraju's Algorithm
6. Tarjan's Algorithm for SCC
7. Tarjan's Algorithm for Bridges
8. Euler Tour Technique
9. Euler Tour for Trees
10. Flattening Trees to Arrays
11. Range Queries on Trees
12. Common Problems
...
> 🧠 **Mental Model:** A bridge is like the **one ferry connecting two islands**. Remove it, and people are stranded! Tarjan's algorithm asks: "Can the island on the other side reach back to the mainland without this ferry?" If not, it's a bridge.
...
If low[v] > disc[u]:
  ...
  → Only path from u to v is the edge (u,v)
  → Removing (u,v) disconnects u and v
  → (u,v) is a bridge!
...
disc = [0, 1, 2, 3]
low  = [0, 0, 0, 0]  ← All can reach vertex 0!
```
...
**1. Root of DFS tree:** Articulation point if it has ≥ 2 children.
```
    0 (root)
   / \
  1   2
...
If low[v] >= disc[u]:
  ...
  → All paths from v's subtree to ancestors pass through u
...
Call dfs(0, -1):
  ...
  Call dfs(1, 0):
    ...
    Check: parent != -1 && low[3] >= disc[1]?
      true && (3 >= 1)? YES → isAP[1] = true (already set)
  ...
  Check: parent == -1 && children > 1?
    false && false? NO → isAP[0] = false

Articulation points: [1, 3]

Verification:
  Remove 1: {0} {2} {3,4} → 3 components ✓
  Remove 3: {0,1,2} {4} → 2 components ✓
```
...
> 🧠 **Mental Model:** SCCs are like **friend circles** in a directed social network. Within an SCC, anyone can DM anyone (directly or through chains). Between SCCs, messages can only flow one way — the "cool kids" SCC might never respond to the "nerds" SCC!
...
Proof sketch:
  If u, v in same SCC of G:
    ...
    - v can reach u in G
  Then in G^T:
    - v can reach u in G^T (reversed u→v)
    - u can reach v in G^T (reversed v→u)
  So same SCC in G^T!
```
...
SCC 2: {2}
  2 can't reach any other vertex ✓
```
...
**Property:** Subtree of u = vertices with `in[v] ∈ [in[u], out[u]]`
...
Subtree of 1: vertices with in ∈ [1, 6]
  → {1, 3, 4} ✓
```
...
2. Sum = path(u → LCA) + path(v → LCA) - value(LCA)
...
| **Removes** | Edge | Vertex |
| **Condition** | `low[v] > disc[u]` | `low[v] >= disc[u]` or root with ≥2 children |
...
|-----------|--------|-------|------------|---------|
| **Kosaraju** | 2 DFS | O(V) + transpose | O(V+E) | Yes ✓ |
...
2. **Articulation points: low[v] >= disc[u] OR root with ≥2 children** - Child can't bypass u
...
6. **Euler path exists: ≤2 odd degree vertices** - Visit every edge once
7. **Euler tour for trees: Record in/out times** - Not same as Euler path!
8. **Subtree of u: vertices with in[v] ∈ [in[u], out[u]]** - Flatten tree to array
9. **Tree queries → array range queries** - Use segment tree/BIT
...
Master these advanced graph algorithms and you'll handle complex connectivity problems! 🎯
