...
> 🧠 **Mental Model:** Think of a heap as a **sports tournament bracket** — the champion (max or min) always sits at the top. Adding a player? They bubble up through matches. Champion leaves? Runner-up takes over after a playoff!
...
> 🧠 **Mental Model:** Imagine a **hospital ER triage system**. The most critical patient (highest priority) is always seen first. New patients are quickly slotted in, and when one is treated, the next most critical rises to the top automatically.
...
**Advantages:**
✅ **O(1) access to min/max** - Root is always min/max  
✅ **O(log n) insert/delete** - Height = log n  
✅ **O(n) build from array** - Heapify is linear!  
✅ **Space efficient** - Array implementation (no pointers)
...
| Parent relation | Parent ≥ Child | Parent ≤ Child |
| Extract | Largest | Smallest |
| Top-K largest | Keep min heap size K | ❌ |
| Top-K smallest | ❌ | Keep min heap size K |
...
maxHeap.top();         // Access max: O(1) → 30
maxHeap.pop();         // Remove max: O(log n)
maxHeap.size();        // Size: O(1) → 2
maxHeap.empty();       // Check empty: O(1) → false
```
...
public:
    void addNum(int num) {
        ...
        // Maintain size: maxHeap has ≥ minHeap
...
Process [15,20]:
  Earliest end = 10 ≤ 15 (start), can reuse! ✓
...
Master these heap patterns and you'll efficiently handle priority-based problems! 🔥
