...
> 🧠 **Mental Model:** Greedy is like being a **kid at a buffet** — always grab the best-looking item RIGHT NOW without worrying about what's coming later. Sometimes this works perfectly (sorted coins), sometimes you regret it (arbitrary coins)!
...
6. Meeting Rooms Problems
7. Huffman Coding
8. Coin Change
9. Advanced Greedy Patterns
10. Common Problems
...
> 🧠 **Mental Model:** The core greedy insight: **"locally optimal + never backtrack = globally optimal"** (if the problem has the right structure). It's like always taking the highway exit that gets you closest to your destination — works if roads are straightforward, fails in a maze!
...
Greedy approach:
  25 (largest ≤ 41): Use 1 quarter, remaining = 16
  10 (largest ≤ 16): Use 1 dime, remaining = 6
  5 (largest ≤ 6): Use 1 nickel, remaining = 1
  1: Use 1 penny
...
Step 2: Pick intervals greedily
  ...
  Check [1,3]: starts at 1 < 2 (overlap) → Skip
  
  Check [2,4]: starts at 2 ≥ 2 ✓
    Pick [2,4]: count=2, lastEnd=4
  
  Check [3,5]: starts at 3 < 4 (overlap) → Skip
...
Selection:
  ...
  Skip B (start=3 < 4)
  Skip C (start=0 < 4)
  Pick D (start=5 ≥ 4, finish=7)
  Skip E, F, G (all start before 7)
  Pick H (start=8 ≥ 7, finish=11)
  Skip I, J (start before 11)
  Pick K (start=12 ≥ 11)

Selected: A, D, H, K → 4 activities ✓
```
...
Process [3,5]:
  Min end = 3 ≤ 3 (start), reuse group 1 ✓
  Remove 3, add 5
  Heap: [4, 5]
  Groups: 2

Process [4,6]:
  Min end = 4 ≤ 4 (start), reuse group 2 ✓
  Remove 4, add 6
  Heap: [5, 6]
  Groups: 2
...
Simulation (two pointers):
  i=0, j=0: starts[0]=0 < ends[0]=10
    Meeting starts, rooms++ → rooms=1, max=1

  i=1, j=0: starts[1]=5 < ends[0]=10
    Meeting starts, rooms++ → rooms=2, max=2

  i=2, j=0: starts[2]=15 > ends[0]=10
    Meeting ends, rooms-- → rooms=1, j=1

  i=2, j=1: starts[2]=15 < ends[1]=20
    Meeting starts, rooms++ → rooms=2
...
Encoding "ABRACADABRA":
  ...
  = 25 bits (vs 11×8=88 bits with ASCII)
```
...
Greedy:
  4: 6/4 = 1, use 1×4, remaining = 2
  3: 2/3 = 0, skip
  1: 2/1 = 2, use 2×1, remaining = 0
...
|-------------|---------------|-----|
| {1, 5, 10, 25} | ✓ Yes | Greedy (O(n)) |
| {1, 2, 5, 10, 20, 50} | ✓ Yes | Greedy |
| {1, 3, 4} | ✗ No | DP (O(amount × n)) |
| {1, 6, 10} | ✗ No | DP |
| Powers of k | ✓ Yes | Greedy |
...
Proof: If A→B fails and total is enough, then B→A must succeed
```
...
|---------|--------|----|-----|
| Activity selection | ✓ | ❌ | Earliest finish optimal |
| Fractional knapsack | ✓ | ❌ | Take highest value/weight |
| 0/1 Knapsack | ❌ | ✓ | Can't split items |
| Coin change (canonical) | ✓ | ❌ | Each coin is multiple |
| Coin change (arbitrary) | ❌ | ✓ | Greedy suboptimal |
| Huffman coding | ✓ | ❌ | Merge smallest optimal |
...
| **Staying ahead** | Show greedy is always ≥ any other solution |
...
1. **Greedy = local optimum → global optimum** - But doesn't always work!
...
Master these greedy patterns and recognize when local choices lead to global optimality! 🎯
