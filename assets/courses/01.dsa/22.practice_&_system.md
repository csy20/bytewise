...
> 🧠 **Mental Model:** Becoming great at CP is like **training for a sport**. Topic sets = drilling fundamentals. Mixed rounds = scrimmages. Timed contests = game day. The goal: convert conscious thinking into automatic pattern recognition, like a chef who "just knows" what spice to add!

---

## Table of Contents
1. Practice Strategy Overview
2. Problem Sets by Topic
3. Mixed Rounds
4. Timed Contests
5. Patterns Checklist
6. Test Generation
7. Debugging Diary
8. Contest Preparation
9. Mental Models
...
**Deliberate Practice:**
```
Topic-focused → Learn patterns deeply
Mixed practice → Build recognition
...
> 🧠 **Mental Model:** Topic sets are like **kata in martial arts** — you practice the same move 100 times until it becomes instinctive. After drilling two pointers for 20 problems, your fingers will type the pattern before your brain finishes reading the problem!
...
□ Max Sum of Rectangle No Larger Than K
  Pattern: 2D → 1D compression + two pointers
```
...
Advanced (3):
  □ Critical Connections in Network (Bridges)
...
> 🧠 **Mental Model:** Mixed rounds are like a **chef facing mystery ingredients** — you don't know what's coming, so you must recognize patterns quickly. Is this a sliding window? A DP? Your brain builds a "pattern index" that fires instantly when you see similar problems.
...
For each problem, identify:
  ...
  2. What are the constraints?
     - n ≤ 10: Try all possibilities (brute force)
     - n ≤ 20: O(2^n) possible (bitmask, meet-in-middle)
     - n ≤ 100: O(n³) possible (Floyd-Warshall, DP)
     - n ≤ 1000: O(n²) possible (DP, nested loops)
     - n ≤ 10⁵: O(n log n) required (sorting, heap, segment tree)
     - n ≤ 10⁶: O(n) or O(n log n) (linear scan, hash, sort)
  
  3. What patterns appear?
     - Sorted array → binary search
     - Tree → DFS/BFS/DP on tree
     - Shortest path → BFS/Dijkstra
     - Subarray/substring → sliding window
     - Distinct elements → hash set
...
Optimization problem?
  ...
  └─ No → Search problem?
           ├─ Yes → BFS/DFS
           └─ No → Data structure problem

Array/String?
  ├─ Subarray/substring → Sliding window
  ├─ Two elements → Two pointers or hash
  └─ Multiple queries → Prefix sum or segment tree

Tree?
  ├─ Path queries → DFS/LCA
  ├─ Subtree queries → DFS/DP on tree
  └─ Level queries → BFS

Graph?
  ├─ Shortest path → BFS/Dijkstra
  ├─ Connectivity → DFS/DSU
...
> 🧠 **Mental Model:** Timed contests are **game day in sports**. All your training converges under pressure. You learn things you can't learn in practice: managing nerves, prioritizing problems, knowing when to abandon ship. The clock is your opponent!
...
```
□ Two pointers
  ↳ Converging (two sum, palindrome)
  ↳ Same direction (remove duplicates)
  ↳ Sliding window (subarray sum)

□ Hash map/set
  ↳ Frequency counting
  ↳ Seen/not seen checking
  ↳ Index mapping
...
Checklist:
  ...
  □ "At most K" → window expansion/contraction
...
Checklist:
  □ Tree → DFS/BFS?
  □ Ancestor relationship → LCA pattern
  □ Binary tree → can use parent pointers or DFS
  
Pattern: DFS with parent tracking or Binary lifting ✓
...
□ Empty input
□ Single element
□ Two elements (minimum for comparisons)
□ All same elements
□ Already sorted (if relevant)
□ Reverse sorted (if relevant)
```

**Category 2: Boundary Cases**
```
□ Minimum constraint (n=1, val=0, etc.)
□ Maximum constraint (n=10^5, val=10^9, etc.)
□ Just below/above threshold
  Example: If n ≤ 1000, test n=999, n=1000, n=1001
```
...
□ Answer is at start/end/middle
□ Answer is minimum/maximum possible
```
...
// Before:
for (int i = 0; i < n; i++) {
    for (int j = i; j < n; j++) { // ← Bug: should be j = i+1
...
// Before:
int product = 1;
for (int x : nums) product *= x; // ← Overflow

// After:
long long product = 1;
for (int x : nums) product *= x; // ← Use long long
```
...
```cpp
// Before:
vector<int> dp(amount + 1); // ← Default to 0

// After:
vector<int> dp(amount + 1, INT_MAX); // ← Initialize to INF
dp[0] = 0;
```
...
□ Checked for overflow:
  ...
  □ Large addition
  □ Array indexing with large numbers
...
□ Code review:
  ...
  □ No integer division where should be float
...
**1 Week Before:**
```
□ Solve topic sets in weak areas
□ Review common patterns
...
```
□ Read editorials for problems you solved
  - Learn alternative solutions
  - Optimize your approach

□ Read editorials for problems you didn't solve
...
```
Wrong Answer → Learn edge case
Time Limit → Learn optimization
Memory Limit → Learn space efficiency
Runtime Error → Learn careful coding
```

**Review cycle:**
```
Solve → Fail → Debug → Learn → Retry → Master
```
...
1. **Three-phase learning: Topic → Mixed → Timed** - Depth, breadth, speed
...
10. **Contest strategy: read all → solve easy → attempt hard** - Time management
...
Build these systems and watch your competitive programming skills soar! 📈
