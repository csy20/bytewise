...
> 🧠 **Mental Model:** Think of a linked list like a **treasure hunt with clues**. Each location (node) has something valuable (data) and a note pointing to the next location (pointer). You must follow the trail — no shortcuts!
...
5. Finding Middle
6. K-Reverse Operations
7. Cycle Detection (Floyd's Algorithm)
...
> 🧠 **Mental Model:** A singly linked list is like **train cars connected with hooks**. You can only walk forward through the cars, and each car only knows about the one behind it. To find something, start at the engine and walk through!
...
> 🧠 **Mental Model:** A doubly linked list is like a **two-way street** where each house has addresses of both neighbors. You can walk forward OR backward, which makes some operations (like deletion) much easier!
...
✅ **LRU Cache** (need to move nodes to front)  
✅ **Music player** (next/previous song)  
✅ **Undo/Redo** (navigate both directions)
...
S F
↓ ↓
1 2 3 4 5 → S moves to 2, F moves to 3
  S   F
  2   3 4 5 → S moves to 3, F moves to 5
    S     F
    3     5 → F->next is NULL, stop
Middle = 3 ✓

Even length [1,2,3,4]:
S F
1 2 3 4 → S moves to 2, F moves to 3
  S   F
  2   3 4 → S moves to 3, F moves to NULL
    S   F
...
**Trace:**
```
List A: [1] → [2] → [7] → [8] → NULL  (length 4)
List B: [3] → [7] → [8] → NULL        (length 3)
...
pA: a1 → a2 → NULL → b1 → b2 → b3 → NULL
pB: b1 → b2 → b3 → NULL → a1 → a2 → NULL

Both reach NULL at same time → pA == pB == NULL
```
...
Step 3: 3 < 4, attach [3]
  dummy → [1] → [2] → [3], l1=[5], l2=[4]

Step 4: 5 > 4, attach [4]
  dummy → [1] → [2] → [3] → [4], l1=[5], l2=[6]

Step 5: 5 < 6, attach [5]
  dummy → [1] → [2] → [3] → [4] → [5], l1=NULL, l2=[6]
...
| Type | Next | Prev | Last→First | Use Case |
|------|------|------|------------|----------|
| Singly | ✓ | ✗ | ✗ | Simple, memory efficient |
| Doubly | ✓ | ✓ | ✗ | Bidirectional traversal |
| Circular | ✓ | ✗ | ✓ | Round-robin, playlists |
| Doubly Circular | ✓ | ✓ | ✓ | Complex navigation |
...
Master these patterns and you'll efficiently solve 95% of linked list problems! 🚀
