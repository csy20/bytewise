...
> 🧠 **Mental Model:** DP is like being a **smart student taking notes**. Instead of re-solving the same problem 100 times, you write down the answer and look it up later. "Those who cannot remember the past are condemned to recompute it!"
...
> 🧠 **Mental Model:** The key insight: if you're solving the same subproblem repeatedly, **store the answer!** Like Fibonacci — you calculate fib(3) dozens of times in the naive approach. DP turns exponential into polynomial by trading space for time.
...
Actually: houses[0] + houses[2] + houses[4] = 12 ✓
```
...
Jump sequence 1: 0→1→2→3
  Cost: |20-10| + |30-20| + |10-30| = 10 + 10 + 20 = 40

Jump sequence 2: 0→1→3
  Cost: |20-10| + |10-20| = 10 + 10 = 20 ✓

Jump sequence 3: 0→2→3
...
dp[6] = 4
  Best: extend from dp[5] → dp[6] = 4
  LIS: [2, 5, 7, 101]

dp[7] = 4
  Best: extend from dp[5] → dp[7] = 4
  LIS: [2, 5, 7, 18]
...
Example:
  ...
  New element 4:
    Can extend [2, 3] → [2, 3, 4] ✓
    Can't extend [2, 5] (4 < 5) ✗
...
3. **LIS O(n²): dp[i] = max LIS ending at i** - Check all j < i
...
11. **LCS: Match → diagonal + 1, no match → max(top, left)** - Two strings
12. **Edit distance: Match → diagonal, no match → min(insert, delete, replace) + 1** - Three operations
13. **Palindrome partitioning: Precompute palindromes** - O(n²) check
...
Master these DP patterns and you'll solve any DP problem! 💡
