...
> 🧠 **Mental Model:** Think of a graph as a **city map** — vertices are locations, edges are roads. BFS explores like **ripples in water** (all neighbors first), DFS explores like a **determined explorer** (go as deep as possible, then backtrack).
...
> 🧠 **Mental Model:** A graph is just **things connected to other things**. Social networks? People connected by friendships. Road networks? Cities connected by highways. Master graphs and you master connections!
...
Edge (A,B) means A↔B (bidirectional)
```
...
Edge A→B means can go from A to B (not B to A)
```
...
| **Path** | Sequence of vertices via edges | A→B→C |
...
✅ Implementing graph algorithms (Floyd-Warshall)  
✅ Small graphs (n ≤ 1000)
...
Adjacency List:
  Space: 1000 + 2×2000 = 5000 integers ≈ 20 KB

Adjacency Matrix:
  Space: 1000×1000 = 1,000,000 integers ≈ 4 MB
...
**Topological ordering:** Linear ordering of vertices where for every edge u→v, u comes before v.
...
DFS from 0:
  ...
  DFS(1):
    ...
    DFS(2):
      ...
      DFS(3):
        state[3] = VISITING
        Check neighbor 0: state[0] = VISITING ✗
        Cycle detected!
```
...
- Transitive: a→b, b→c implies a can reach c
...
Master these fundamentals and you'll handle any graph traversal or topological sort problem! 🗺️
