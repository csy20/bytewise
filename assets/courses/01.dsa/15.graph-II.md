...
> 🧠 **Mental Model:** Shortest path = **finding the cheapest flight route**. Dijkstra sends scouts that always report back from the nearest unexplored city. Bellman-Ford broadcasts distance updates like ripples. MST = connecting cities with minimum total cable cost!

---

## Table of Contents
1. Shortest Path Overview
...
|---------|-----------|------------|-------------|
| **Single-source, non-negative** | Dijkstra | O(E log V) | Weights ≥ 0 |
| **Single-source, 0-1 weights** | 0-1 BFS | O(V + E) | Weights ∈ {0, 1} |
...
✅ Need shortest path tree  
❌ **FAILS with negative weights!**

**0-1 BFS:**
✅ Weights are only 0 or 1  
✅ Faster than Dijkstra for this case  
✅ Common in grid problems
...
> 🧠 **Mental Model:** Imagine you're the **king sending scouts** to explore your kingdom. You always ask: "Who's the closest unexplored location?" Send a scout there, they report back all roads they find. Each scout's report is final — no one can find a shorter path later (because roads aren't negative!)
...
Proof by contradiction:
  Assume better path exists: source → ... → x → u
...
Step 1: Extract (0, 0)
  Process vertex 0
  Relax edges:
    0→1 weight 1: dist[1] = min(∞, 0+1) = 1
    0→2 weight 4: dist[2] = min(∞, 0+4) = 4
  dist = [0, 1, 4, ∞]
  pq = [(1, 1), (4, 2)]
...
Step 2: Extract (1, 1)
  Process vertex 1
  Relax edges:
    1→0 weight 1: dist[0] = min(0, 1+1) = 0 (no change)
    1→3 weight 2: dist[3] = min(∞, 1+2) = 3
  dist = [0, 1, 4, 3]
  pq = [(3, 3), (4, 2)]
...
Step 3: Extract (3, 3)
  Process vertex 3
  Relax edges:
    3→1 weight 2: dist[1] = min(1, 3+2) = 1 (no change)
    3→2 weight 1: dist[2] = min(4, 3+1) = 4 (no change)
  dist = [0, 1, 4, 3]
  pq = [(4, 2)]
...
Step 4: Extract (4, 2)
  Process vertex 2
  Relax edges:
    2→0 weight 4: dist[0] = min(0, 4+4) = 0 (no change)
    2→3 weight 1: dist[3] = min(3, 4+1) = 3 (no change)
  dist = [0, 1, 4, 3]
  pq = []
...
When we process vertex with dist=d:
  - Edge weight 0 → dist=d → add to front
  - Edge weight 1 → dist=d+1 → add to back
...
Step 1: Pop front = 0
  Edges:
    0→1 weight 0: dist[1] = 0+0 = 0, push_front(1)
    0→2 weight 1: dist[2] = 0+1 = 1, push_back(2)
  dist = [0, 0, 1, ∞]
  dq = [1, 2]  (1 at front!)

Step 2: Pop front = 1
  Edges:
    1→3 weight 0: dist[3] = 0+0 = 0, push_front(3)
  dist = [0, 0, 1, 0]
  dq = [3, 2]
...
Step 4: Pop front = 2
  Edges:
    2→3 weight 1: dist[3] = min(0, 1+1) = 0 (no change)
  dq = []
...
> 🧠 **Mental Model:** Think of Bellman-Ford as **passing rumors through a crowd**. Each round, everyone tells their neighbors what they know. After n-1 rounds, everyone knows the truth! If rumors still change after that... someone's lying (negative cycle).
...
Dijkstra picks 0→1 (cost 5) as final.
But 0→2→? might be better if negative edges exist!
```
...
**Why V-1 iterations?**
```
Shortest path from source to any vertex uses ≤ V-1 edges.
...
Cycle: 1→2→0→1 = -3 + 1 + 2 = 0? No wait:
Actually: 1→2 (-3), 2→0 (doesn't exist)
...
Cycle: 0→1→2→0 = 1 + (-1) + (-2) = -2 (negative!)

After V-1 iterations, can still improve:
  dist[0] can decrease again → negative cycle!
```
...
**Alternatives:**
- Run Dijkstra V times: O(V²E log V) ≈ O(V³ log V)
- Run Bellman-Ford V times: O(V²E) ≈ O(V³)
...
Try adding k as intermediate:
  If i→k→j is shorter than current i→j, update!
```
...
k=0 (use vertex 0 as intermediate):
  Check all pairs (i,j):
    dist[1][2] = min(∞, dist[1][0] + dist[0][2]) = min(∞, ∞) = ∞
...
> 🧠 **Mental Model:** You're a **telecommunications company** connecting cities with fiber optic cables. You must connect ALL cities (spanning), no redundant loops (tree), and minimize total cable cost (minimum). MST algorithms help you find the cheapest way to wire everyone together!
...
All spanning trees:
  ...
  Tree 2: edges (0,1,1), (1,3,2), (2,3,3) = weight 6 ✓
  ... many others
...
This edge crosses some cut → must be in MST.
```

**Time:** O(E log E + E α(V)) ≈ O(E log E)
- Sort edges: O(E log E)
- E union-find operations: O(E α(V)) ≈ O(E)
- α(V) is inverse Ackermann, effectively constant
...
find(0) walks through 1→0 = O(1)
find(4) walks through 1→4 = O(1)
...
- α(n) ≤ 4 for all practical n (even n = 2^65536)
- **Effectively constant time!**

**Proof (simplified):**
```
Union by rank: O(log n) height
Path compression: Amortizes to O(α(n))

Combined: O(α(n)) amortized per operation

For n operations: O(n α(n)) total
```
...
int networkDelayTime(vector<vector<int>>& times, int n, int k) {
    ...
    for (auto& t : times) {
        adj[t[0]].push_back({t[1], t[2]});  // u → v, weight w
    }
...
| **Path compression** | O(α(n)) amortized | Flattened paths |
| **Both** | O(α(n)) amortized | Nearly constant! |
...
2. **0-1 BFS uses deque: weight 0 → front, weight 1 → back** - O(V+E) instead of O(E log V)
3. **Bellman-Ford relaxes V-1 times** - Shortest path uses ≤ V-1 edges
...
9. **DSU with union by rank: Attach smaller tree to larger** - Height ≤ log n
10. **DSU with path compression: Point all to root during find** - Amortized O(α(n))
11. **Combined DSU optimizations: Effectively O(1)** - α(n) ≤ 4 for all practical n
12. **Floyd-Warshall finds all-pairs in O(V³)** - Better than V runs of Dijkstra for dense graphs

Master these algorithms and you'll handle any shortest path or MST problem! 🚀
