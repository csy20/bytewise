...
> 🧠 **Mental Model:** These techniques are like using **two hands to shrink/stretch a rubber band**. Two pointers pinch from both ends or crawl together; sliding window drags a "view frame" across data. Both convert O(n²) brute force into elegant O(n) solutions!

---

## Table of Contents
1. Two Pointer Fundamentals
2. Pair/Sum Problems
3. Dedupe Runs
4. Sliding Window Fundamentals
5. Fixed Window
6. Variable Window
7. Window with Counts
8. The "At Most K" Trick
9. Advanced Patterns
...
> 🧠 **Mental Model:** Think of two pointers like **two fingers on a sorted deck of cards**. One finger on smallest, one on largest. Need bigger sum? Move left finger right. Need smaller? Move right finger left. You're guaranteed to find the answer without checking every pair!

**Concept:** Use two pointers to traverse data structure, usually to reduce time complexity from O(n²) to O(n).
...
✅ Need to partition/reorder elements  
...
sum = 2 + 5 = 7 ✓ → pairs: [(1,6), (2,5)]

arr = [1, 1, 2, 3, 4, 4, 5, 6]
                L     R

sum = 3 + 4 = 7 ✓ → pairs: [(1,6), (2,5), (3,4)]
...
**Problem:** "abbaca" → "ca" (remove all consecutive duplicates repeatedly)
...
> 🧠 **Mental Model:** Imagine looking through a **train window at passing scenery**. The window (your view) slides along, and you only need to note what enters and exits the frame — not re-examine everything inside. That's O(1) per slide instead of O(window_size)!
...
i=3: Add 6, remove 1 → [3, 2, 6], sum = 11, max = 11
i=4: Add -1, remove 3 → [2, 6, -1], sum = 7, max = 11
i=5: Add 4, remove 2 → [6, -1, 4], sum = 9, max = 11
i=6: Add 1, remove 6 → [-1, 4, 1], sum = 4, max = 11
i=7: Add 8, remove -1 → [4, 1, 8], sum = 13, max = 13
i=8: Add 2, remove 4 → [1, 8, 2], sum = 11, max = 13
...
Window [AAB]: size=3, maxCount('A')=2, replacements=1 ✓
Window [AABA]: size=4, maxCount('A')=3, replacements=1 ✓
Window [AABAB]: size=5, maxCount('A')=3, replacements=2 ✗
Shrink to [ABAB]: size=4, maxCount('A')=2, replacements=2 ✗
Shrink to [BAB]: size=3, maxCount('B')=2, replacements=1 ✓
...
...
Master these patterns and you'll solve 90% of two-pointer and sliding window problems! 🚀
