...
> 🧠 **Mental Model:** Think of a hash table as a **magical library catalog**. Instead of searching every shelf for a book, you compute a "code" from the title that tells you exactly which shelf to check. O(1) lookup instead of O(n) — that's the power of hashing!

---

## Table of Contents
1. Hash Table Fundamentals
2. unordered_map Basics
3. Counting Patterns
4. Deduplication
5. Grouping Patterns
...
> 🧠 **Mental Model:** Imagine a **coat check at a theater**. You hand over your coat (value), get a numbered ticket (hash). To retrieve, you don't search all coats — the number tells them exactly where to look!
...
Example:
"apple" → hash("apple") → 12345 → 12345 % 10 = 5 → arr[5] = price of apple
```
...
Naive: "abc" = 97+98+99 = 294
       "bca" = 98+99+97 = 294  ❌ COLLISION!

Better: "abc" = 97*31² + 98*31 + 99 = 93554
        "bca" = 98*31² + 99*31 + 97 = 97364  ✅ Different!
```
...
buckets[0]: nullptr
buckets[1]: ["apple", 5] → nullptr
buckets[2]: ["banana", 3] → ["berry", 7] → nullptr  (collision!)
buckets[3]: nullptr
buckets[4]: ["cherry", 2] → nullptr
```
...
> 🧠 **Mental Model:** Counting with maps is like a **tally counter for each unique item**. As you scan through data, you click the counter for that item. At the end, instant access to any count!
...
int subarraySum(vector<int>& arr, int k) {
    unordered_map<int, int> prefixCount;  // sum → count
...
i=0: 'e'→'a', 'a'→'e', maps = {e:a}, {a:e}
i=1: 'g'→'d', 'd'→'g', maps = {e:a, g:d}, {a:e, d:g}
...
i=0: 'f'→'b', maps = {f:b}, {b:f}
i=1: 'o'→'a', maps = {f:b, o:a}, {b:f, a:o}
i=2: 'o'→'r', but 'o' already maps to 'a' ✗
...
"abc": diff = ['b'-'a', 'c'-'b'] = [1, 1] → key = "1,1,"
"bcd": diff = ['c'-'b', 'd'-'c'] = [1, 1] → key = "1,1,"
"xyz": diff = ['y'-'x', 'z'-'y'] = [1, 1] → key = "1,1,"
...
**Definition:** α = n / m (number of elements / number of buckets)

**Impact:**
- α < 0.7: Good performance
- α > 1.0: Many collisions, slower operations

**Rehashing:** When α exceeds threshold (usually 0.75), create larger table and rehash all elements.
...
cout << map["nonexistent"] << "\n";  // Prints 0, but adds "nonexistent" → 0
...
map["a"]++;       // OK
map["b"]++;       // Creates "b" → 0, then increments to 1
...
Master hashing and you'll solve counting/grouping problems in O(n) time! 🚀
