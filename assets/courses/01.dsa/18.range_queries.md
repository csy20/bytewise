...
> 🧠 **Mental Model:** Range query structures are like **pre-computed cheat sheets** at different granularities. Sparse Table = answer cards for every power-of-2 range. BIT = clever binary-sized buckets. Segment Tree = a divide-and-conquer hierarchy where every node knows about its range!

---

## Table of Contents
1. Range Query Overview
2. Sparse Table
...
6. BIT Construction
7. BIT: Prefix Sums
8. BIT: Kth Smallest by Frequency
9. Segment Tree
10. Segment Tree: Point & Range Updates
11. Lazy Propagation
12. Common Problems
...
> 🧠 **Mental Model:** Imagine you're a **librarian with pre-written index cards**. You have cards for "min of first 1 book", "min of first 2 books", "min of first 4 books", etc. Any range query? Just overlap two cards! Since min(x,x)=x, overlapping doesn't double-count.
...
> 🧠 **Mental Model:** Think of BIT as **nested Russian dolls with sums**. Position 8 (1000₂) holds sum of 8 elements. Position 6 (110₂) holds sum of 2 elements. The binary patterns tell you exactly which dolls to open to get any prefix sum!
...
> 🧠 **Mental Model:** A segment tree is like a **corporate org chart** — the CEO (root) knows company-wide stats, VPs know divisional stats, managers know team stats, down to individual employees (leaves). Any question about a range? Walk down to the right nodes and combine their answers!
