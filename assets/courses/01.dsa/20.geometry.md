...
> 🧠 **Mental Model:** Geometry algorithms turn visual intuition into precise computation. Cross product = right-hand rule for "which way am I turning?" Convex hull = stretching a rubber band around nails. These translate "I can see it" into "the computer can calculate it."

---

## Table of Contents
1. Geometry Overview
2. Point and Vector Basics
3. Orientation (CCW Test)
4. Area Calculations
5. Distance Formulas
6. Line Segment Intersection
7. Convex Hull: Graham Scan
8. Convex Hull: Monotone Chain
9. Common Problems
...
```
A × B = |A| |B| sin(θ)
      = A.x × B.y - A.y × B.x  (z-component in 2D)

Properties:
  - A × B > 0 → B is counterclockwise from A
  - A × B = 0 → collinear
  - A × B < 0 → B is clockwise from A
```
...
> 🧠 **Mental Model:** Think of orientation as **driving directions**. At point P, you're heading toward Q. When you reach Q and look toward R: are you turning left (CCW), right (CW), or going straight (collinear)? The cross product is your GPS telling you which way you're steering!
...
Cross product = PQ × PR = (Q.x - P.x) × (R.y - P.y) - (Q.y - P.y) × (R.x - P.x)
...
Cross product gives signed area of parallelogram:
  - Positive area → left turn
  - Negative area → right turn
  - Zero area → points on line
```
...
R is to the left of line PQ → CCW ✓
```
...
ab · ap = 4 × 2 + 0 × 3 = 8 > 0  (not before A)
ab · bp = 4 × (-2) + 0 × 3 = -8 < 0  (not after B)

Projection is on segment, use pointToLine:

area = |ab × ap| = |4 × 3 - 0 × 2| = 12
|ab| = sqrt(16) = 4
distance = 12 / 4 = 3 ✓
...
Check orientation(p1, q1, p2) and orientation(p1, q1, q2):
  - If different → p2 and q2 are on opposite sides of line p1-q1
  
Check orientation(p2, q2, p1) and orientation(p2, q2, q1):
  - If different → p1 and q1 are on opposite sides of line p2-q2

Both conditions → segments intersect!
```
...
o1 = orientation(p1, q1, p2):
  PQ = (4, 4), PR = (0, 4)
  cross = 4×4 - 4×0 = 16 > 0 → CCW (1)

o2 = orientation(p1, q1, q2):
  PQ = (4, 4), PR = (4, 0)
  cross = 4×0 - 4×4 = -16 < 0 → CW (2)

o3 = orientation(p2, q2, p1):
  PQ = (4, -4), PR = (0, -4)
  cross = 4×(-4) - (-4)×0 = -16 < 0 → CW (2)

o4 = orientation(p2, q2, q1):
  PQ = (4, -4), PR = (4, 0)
  cross = 4×0 - (-4)×4 = 16 > 0 → CCW (1)
...
> 🧠 **Mental Model:** Imagine **wrapping a rubber band around pushpins**. The rubber band snaps to the outermost pins, skipping interior ones. Graham Scan simulates this: sort pins by angle, then walk around adding pins — if we ever turn right, pop the previous pin (rubber band would skip it)!
...
| **Dot product** | x₁x₂ + y₁y₂ | O(1) |
| **Cross product** | x₁y₂ - x₂y₁ | O(1) |
...
2. **Cross product = 2 × triangle area** - Signed area
...
6. **Orientation test: (Q-P) × (R-P)** - Which side of line PQ is R?
...
Master these geometry fundamentals and you'll handle any computational geometry problem! 📐
