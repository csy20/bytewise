...
> 🧠 **Mental Model:** These are the **power-ups** that turn "impossible" into "elegant". Meet in the Middle = two search parties covering ground faster together. Binary Lifting = express elevators stopping at floors 1, 2, 4, 8... DSU on Tree = inheriting data from your biggest child instead of starting from scratch!
...
3. Meet in the Middle: Applications
4. Binary Lifting
5. Binary Lifting: LCA
6. Binary Lifting: Kth Ancestor
7. DSU on Tree
8. Policy-Based Data Structures
9. Order Statistics Tree
10. Common Problems
...
|-----------|--------------|-------------|
| **Meet in the Middle** | O(2^n) → O(2^(n/2)) | Subset problems |
| **Binary Lifting** | O(n) → O(log n) | Tree queries |
| **DSU on Tree** | O(n²) → O(n log n) | Subtree queries |

### When to Use Each

**Meet in the Middle:**
✅ Subset sum/search problems  
✅ n ≤ 40 (too large for 2^n)  
✅ Can split into two independent parts
...
✅ Subtree statistics  
✅ Color counting  
✅ Distinct elements in subtree

---

## Meet in the Middle

> 🧠 **Mental Model:** Instead of one person searching a 40-floor building (2^40 doors), **two people** each search 20 floors (2×2^20 doors) and meet to compare notes. The exponential becomes... less exponential!

### What is Meet in the Middle?

**Optimization:** Split problem into two halves, solve separately, combine.

**Key insight:** 2^40 is too large, but 2^20 × 2 is manageable!
...
**Naive:** Try all 2^n subsets → too slow for n=40
...
For target 10:
  Left sum 0 → need 10 from right ✓ (exists: {4,6})
...
Right subsets (2^2 = 4):
  ...
  mask=01: sum = 5, needed = 5
    Binary search 5 in [0,2,3,5]? YES ✓
...
needed = 10 - 5 = 5
5 exists in leftSums ✓

Solution: {2,3} ∪ {5} = {2,3,5} with sum 10 ✓
```
...
> 🧠 **Mental Model:** Normal elevators stop at every floor. Binary Lifting is like having **express elevators** that go 1, 2, 4, 8, 16... floors at once. To go up 13 floors? Take the 8-floor express, then 4-floor, then 1-floor. Any distance in log(n) jumps!
...
> 🧠 **Mental Model:** Imagine **inheriting your parent's business**. Instead of rebuilding from scratch, the largest child passes on their entire operation. Smaller siblings start fresh, but their work merges in. Total rebuilding? Only O(log n) times per person!
...
**Naive:** For each node, DFS entire subtree → O(n²)

**DSU on Tree:** Reuse calculations from heavy child → O(n log n)

### How DSU on Tree Works

**Key insight:**
```
Light children: small subtrees → count separately
Heavy child: largest subtree → keep its data!
...
(Each time it moves up, parent's subtree ≥ 2× larger)
...
// Count elements less than x
int cnt = s.order_of_key(6);   // Elements < 6 → 3 (1,2,5)
cout << cnt << endl;
...
|-----------|------|-----|-------------|
| **Meet in Middle** | O(2^n) | O(2^(n/2)) | Subset problems, n ≤ 40 |
...
1. **Meet in middle splits exponential search** - O(2^n) → O(2^(n/2))
...
Master these advanced techniques and you'll solve complex competitive programming problems efficiently! 🚀
