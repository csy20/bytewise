...
> 🧠 **Mental Model:** Sorting is like **organizing a messy bookshelf** — there are many strategies (by author, by color, by size), and the best one depends on your situation. Once sorted, finding any book becomes trivial — that's the power of binary search!
...
3. Merge Sort
4. Quick Sort
5. std::sort & Custom Comparators
6. Binary Search Basics
7. Binary Search on Values
8. Binary Search on Answer
9. Boundary Patterns (lower_bound/upper_bound)
...
**Steps:**
1. Find minimum in entire array → swap with position 0
2. Find minimum in array[1..n] → swap with position 1
...
> 🧠 **Mental Model:** Merge sort is like organizing a huge pile of papers. **Split them into smaller piles, sort each pile, then merge them back** carefully. Even if you have 1000 papers, sorting 10 piles of 100 and merging is faster than sorting all 1000 at once!
...
> 🧠 **Mental Model:** Quick sort is like **sorting mail at a post office**. Pick a "pivot" letter (say 'M'). All letters A-L go left, N-Z go right. Now each pile is smaller, repeat! The key is picking a good pivot.
...
**Disadvantages:**
✗ Not stable  
✗ Worst case O(n²) (mitigated with random pivot)  
...
> 🧠 **Mental Model:** Binary search is like finding a word in a **dictionary**. You don't start at page 1 — you flip to the middle, check if your word is before or after, then repeat in that half. Each flip eliminates half the remaining pages!
...
**Trace:**
```
x = 8, answer should be 2 (since 2² = 4 ≤ 8, 3² = 9 > 8)
...
**lower_bound(x):** First position where element ≥ x can be inserted  
...
|-----------|----------------------|-------|--------|----------|
| Selection | O(n²)/O(n²)/O(n²) | O(1) | ✗ | Small arrays, min swaps |
| Insertion | O(n)/O(n²)/O(n²) | O(1) | ✓ | Nearly sorted, online |
| Merge | O(n log n)/O(n log n)/O(n log n) | O(n) | ✓ | Linked lists, stable |
| Quick | O(n log n)/O(n log n)/O(n²) | O(log n) | ✗ | General purpose |
| Heap | O(n log n)/O(n log n)/O(n log n) | O(1) | ✗ | In-place O(n log n) |
| std::sort | O(n log n)/O(n log n)/O(n log n) | O(log n) | ✗ | Default choice |
...
| lower_bound | [0, n] | arr[mid] < target | First ≥ target |
...
Master these sorting and binary search patterns and you'll efficiently solve 90% of related problems! 🚀
