...
> 🧠 **Mental Model:** Think of C++ as a **high-performance sports car** — it gives you incredible speed and control, but you need to understand both the engine (memory management) and the dashboard (syntax/STL) to drive it effectively. Unlike automatic cars (Python/Java), you're in control of the gears!

---

## Table of Contents
1. Tooling & Compilation
2. Fast I/O
3. Debugging Techniques
4. Templates
5. Lambdas
6. References & Move Semantics
7. STL Containers Deep Dive
8. Big-O Complexity
9. Recurrences & Master Theorem
10. Memory Model
11. Integer Overflow
...
> 🧠 **Mental Model:** Think of Fast I/O like an **express checkout lane** at a supermarket. By default, `cin/cout` wait for the slow lane (`scanf/printf`) to sync up. Disabling sync is like opening your own express lane — much faster!
...
> 🧠 **Mental Model:** Think of STL containers as a **toolbox with specialized compartments**. Each container is designed for specific access patterns — just like you wouldn't use a hammer to screw in a bolt. Choosing the right container is half the battle!
...
> 🧠 **Mental Model:** Think of Big-O as **estimating travel time**. You don't count every traffic light — you just say "it's a 2-hour drive." Big-O ignores small details (constants) and focuses on how time grows with distance (input size).
...
```cpp
// Maximum recursion depth ≈ 10^6 (depends on system)
...
> 🧠 **Mental Model:** Think of **Stack as your desk** (fast access, limited space, automatically cleaned) and **Heap as a filing cabinet** (more space, slower access, you must file and retrieve items yourself).
...
> 🧠 **Mental Model:** Think of an `int` as a **car odometer with limited digits**. After 999,999 km, it wraps back to 000,000. Similarly, when an `int` exceeds its max value (~2.1 billion), it "wraps around" to negative numbers!
...
**Why:** `int` range is -2^31 to 2^31-1 (about ±2.1 billion)
...
| Type | Bytes | Range |
|------|-------|-------|
| `int` | 4 | -2^31 to 2^31-1 (~±2.1e9) |
| `long long` | 8 | -2^63 to 2^63-1 (~±9.2e18) |
...
## 🐞 Debug Corner: Common C++ Pitfalls

### Bug #1: Fast I/O Mixed with C-style I/O
...
int main() {
    ...
    int n;
    scanf("%d", &n);  // BUG: Mixing scanf after disabling sync!
    cout << n << "\n";
}
```

**🔍 The Bug:** After `sync_with_stdio(false)`, you **cannot** mix `cin/cout` with `scanf/printf`. The streams are no longer synchronized!
...
int main() {
    ...
    int n;
    cin >> n;  // Use ONLY cin/cout after disabling sync
    cout << n << "\n";
}
```
...
```cpp
vector<int> v;
cout << v[0];     // BUG: Undefined behavior!
cout << v.back(); // BUG: Undefined behavior!

stack<int> st;
cout << st.top(); // BUG: Undefined behavior!
```

**🔍 The Bug:** Accessing elements of an empty container is undefined behavior. It might work, crash, or return garbage.
...
### Bug #3: Integer Overflow in Intermediate Calculation

**❌ Buggy Implementation:**
```cpp
int n = 100000;
int result = n * n;  // BUG: n*n = 10^10, overflows int!

int mid = (left + right) / 2;  // BUG: left+right can overflow!
```

**🔍 The Bug:** The multiplication happens as `int * int = int` (overflow), THEN assigns to result.

**✅ Correct Implementation:**
```cpp
int n = 100000;
long long result = (long long)n * n;  // Cast BEFORE multiply
...
### Bug #4: Using [] on map Creates Default Entry

**❌ Buggy Implementation:**
```cpp
map<string, int> m;
if (m["key"] > 0) {  // BUG: Creates m["key"] = 0 if doesn't exist!
    cout << "Found";
}
cout << m.size();  // Prints 1, not 0!
```

**🔍 The Bug:** Using `operator[]` on a map creates the key with default value if it doesn't exist.

**✅ Correct Implementation:**
```cpp
map<string, int> m;
if (m.count("key") && m["key"] > 0) {  // Check existence first
    cout << "Found";
}
// OR
...
│   ├─ Need sorted keys? ──────────────► map (O(log n) ops)
│   └─ Just fast lookup? ─────────────► unordered_map (O(1) avg)
│
├─ Need only unique values?
│   ├─ Need sorted? ──────────────────► set (O(log n) ops)
│   └─ Just fast lookup? ─────────────► unordered_set (O(1) avg)
│
├─ Need to maintain order of insertion?
│   ├─ Access by index? ──────────────► vector
│   ├─ Insert/delete at both ends? ───► deque
│   ├─ Insert/delete in middle often? ► list
│   └─ Fixed size known at compile? ──► array
│
├─ Need special access pattern?
│   ├─ LIFO (last in, first out)? ────► stack
│   ├─ FIFO (first in, first out)? ───► queue
│   └─ Always need min or max? ───────► priority_queue
│
...
| n ≤ 100,000 | O(n log n) | Sorting, set/map |
| n ≤ 1,000,000 | O(n) | Linear scan, prefix sum |
...
This covers all the fundamental C++ concepts you need for competitive programming! Practice using these concepts, and they'll become second nature. Good luck! 🚀
