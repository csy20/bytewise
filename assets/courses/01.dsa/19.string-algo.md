...
> 🧠 **Mental Model:** String algorithms exploit **pattern self-similarity**. KMP uses a "smart bookmark" that never re-reads text. Z-algorithm measures how far each position "mirrors" the beginning. Rolling hash treats strings as numbers you can slide! Understanding these transforms O(n×m) into O(n+m).

---

## Table of Contents
1. String Algorithm Overview
2. KMP Algorithm
3. KMP: LPS Array Construction
4. KMP: Pattern Matching
5. Z-Algorithm
6. Z-Algorithm: Pattern Search
7. Rolling Hash (Rabin-Karp)
8. Hash Collision Handling
9. Trie (Prefix Tree)
10. Trie: Prefix Queries
11. Manacher's Algorithm
12. Suffix Array Introduction
13. Common Problems
...
**Naive approach:** Check every position → O(nm)
...
| **Trie** | O(mk) | O(n) | O(ALPHABET×nodes) | Multiple patterns, prefix queries |
...
✅ Understanding string borders/periods

**Z-Algorithm:**
✅ Simpler than KMP  
✅ Multiple applications (palindromes, periods)  
...
> 🧠 **Mental Model:** KMP is like having a **smart bookmark** in a book. When you find a mismatch, instead of going back to page 1, the bookmark (LPS array) tells you: "You've already matched these pages — jump to the right spot!" You never re-read the same paragraph twice.
...
Borders for each prefix:
  "A"     → border: "" (length 0)
  "AB"    → border: "" (length 0)
  "ABA"   → border: "A" (length 1)
  "ABAB"  → border: "AB" (length 2)
  "ABABC" → border: "" (length 0)
```
...
i=5, j=5: text[5]='C' ≠ pattern[5]='D', mismatch!
...
i=8, j=5: text[8]='C' ≠ pattern[5]='D', mismatch!
...
> 🧠 **Mental Model:** The Z-array is like holding a **measuring tape from the start of the string**. At each position, you measure: "How many characters here match the beginning?" It's like asking "How well does this position rhyme with the start?"
...
Z[1] = 0: "BACABA" vs "ABACABA" → no match
Z[2] = 1: "ACABA" vs "ABACABA" → "A" matches
Z[3] = 0: "CABA" vs "ABACABA" → no match
Z[4] = 3: "ABA" vs "ABACABA" → "ABA" matches
Z[5] = 0: "BA" vs "ABACABA" → no match
Z[6] = 1: "A" vs "ABACABA" → "A" matches
```
...
Z array:
  ...
  z[2] = 1
  z[3] = 0 (separator)
  z[4] = 3  ← "ABA" matches pattern!
  z[5] = 0
  z[6] = 1
  z[7] = 0
  z[8] = 3  ← "ABA" matches pattern!
  z[9] = 0
  z[A] = 1

Matches at positions:
  z[4] = 3 → text position 4 - 3 - 1 = 0
  z[8] = 3 → text position 8 - 3 - 1 = 4
...
> 🧠 **Mental Model:** Rolling hash treats each string window as a **unique fingerprint number**. Sliding the window? Instead of recalculating from scratch, you just update the fingerprint: pop the old character, push the new one. It's like a calculator with a "shift" button!
...
**Idea:**
```
Hash of "ABC" = 1×31² + 2×31¹ + 3×31⁰ (treat as base-31 number)

Slide window right: "ABC" → "BCD"
  Remove 'A': subtract 1×31²
...
Pattern hash:
  hash('ABC') = 1×31² + 2×31¹ + 3×31⁰ mod M
              = 961 + 62 + 3
              = 1026

Window 0: "ABC"
  hash = 1026
  Match! (hash equals pattern, verify string → true)
  
Roll to window 1: "BCD"
  Remove 'A': hash = (1026 - 1×31²) mod M = 65
  Shift and add 'D': hash = (65×31 + 4) mod M = 2019
...
Roll to window 2: "CDA"
  Remove 'B': hash = (2019 - 2×31²) mod M
  Continue...

Roll to window 4: "ABC"
  hash = 1026
  Match! (verify string → true)
...
For M = 10^9:
  n = 10^4 → P ≈ 0.00005 (very low)
  n = 10^5 → P ≈ 0.5 (HIGH!)
```

**Double hashing:**
```
P(collision) ≈ (n²/(2M1)) × (n²/(2M2))

With M1 = M2 = 10^9:
  n = 10^5 → P ≈ 0.5 × 0.5 × 10^-9 ≈ 10^-9 (negligible!)
```
...
**Key takeaways:**
- Always use large prime modulo (≥ 10^9)
...
Array: children[26] for 'a'-'z'
  ✓ O(1) access
  ✗ Space: always 26 pointers per node
  
Map: only store existing children
  ✓ Space: only used characters
  ✗ O(log k) access (k = # of children)
...
|-----------|---------------|--------|-------|------------------|
| **KMP** | O(m) | O(n) | O(m) | ✓ Yes |
| **Z-Algorithm** | - | O(n+m) | O(n+m) | ✓ Yes |
| **Rabin-Karp** | O(m) | O(n+m) avg | O(1) | ✗ Worst O(nm) |
| **Boyer-Moore** | O(m+σ) | O(n/m) avg | O(m+σ) | ✗ Worst O(nm) |
...
3. **KMP never backtracks in text** - i only increases → O(n)
...
6. **Pattern search with Z: concatenate pattern + "$" + text** - Find Z ≥ m
...
Master these string algorithms and you'll handle any text processing problem efficiently! 🔤
