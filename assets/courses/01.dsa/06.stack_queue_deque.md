...
> 🧠 **Mental Model:** These three are your **fundamental containers with access rules**. Stack = plate stack (LIFO), Queue = waiting line (FIFO), Deque = VIP corridor (both ends). Master these and you'll solve parentheses, histograms, and sliding window problems with ease!
...
3. Deque Fundamentals
4. Parentheses Problems
5. Span Problems
6. Next Greater Element
7. Monotonic Stack
8. Monotonic Deque
9. Range Maximums & Sliding Window
...
> 🧠 **Mental Model:** Think of a stack like a **stack of plates at a buffet**. You can only add or remove from the top. The last plate placed is the first one taken — Last In, First Out (LIFO).
...
✅ Finding nearest smaller/greater elements

---

## Queue Fundamentals

> 🧠 **Mental Model:** Think of a queue like a **line at a coffee shop**. First person in line gets served first. Fair and orderly — First In, First Out (FIFO).
...
> 🧠 **Mental Model:** Think of a deque as a **tunnel with doors on both ends**. You can enter/exit from either side. Perfect for when you need both stack AND queue behavior!
...
✅ Maintain elements in sorted order with both-end access  
...
**Span:** For each element, count how many consecutive previous elements are ≤ current element.
...
80:  80 < 100 → span = 1
60:  60 < 80 < 100 → span = 1
70:  70 > 60 → span = 2 ([60, 70])
60:  60 < 70 → span = 1
75:  75 > 60, 70, 60 → span = 4 ([60, 70, 60, 75])
85:  85 > all previous → span = 6
```
...
i=5 (75): 60≤75, pop 4, stack=[0,1,3]
          70≤75, pop 3, stack=[0,1]
          80>75, stack=[0,1]
          span[5] = 5-1 = 4, stack=[0,1,5]

i=6 (85): 75≤85, pop 5, stack=[0,1]
          80≤85, pop 1, stack=[0]
          100>85, stack=[0]
...
- When we pop elements ≤ current price, we're skipping over them
...
> 🧠 **Mental Model:** Think of a monotonic stack like **bouncers at a club** with a height rule. For monotonic increasing: "You can only enter if you're taller than everyone inside." Shorter people get kicked out! This maintains order and helps find "next greater" elements.
...
**Two types:**
1. **Monotonic Increasing:** Each element ≥ previous
2. **Monotonic Decreasing:** Each element ≤ previous
...
### Pattern 2: Shortest Subarray with Sum ≥ K
...
6. **Each element pushed/popped once** → O(n) total
...
Master these patterns and you'll efficiently solve 90% of stack/queue/deque problems! 🚀
