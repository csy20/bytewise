...
> 🧠 **Mental Model:** Think of a tree like a **family tree** — one ancestor (root) at top, branches of descendants below. Unlike linked lists (one path), trees branch out, enabling O(log n) operations when balanced!
...
> 🧠 **Mental Model:** Think of traversals as **how you read a book**:
> - **Preorder** = Read chapter title first, then contents (root first)
> - **Inorder** = Read left margin notes, main text, right margin (sorted for BST!)
> - **Postorder** = Read all footnotes before conclusions (children before parent)
...
```
Preorder:  ROOT first     → Use for copying tree
Inorder:   ROOT middle    → Use for BST sorting
Postorder: ROOT last      → Use for deleting tree
```
...
st1 does reverse postorder (Root → Right → Left):
...
Invalid BST:
  ...
  2   6
 / \
1  5  ← 5 > 4, should be in right subtree ✗
```
...
// 6 < 15 ✓ but 6 < 10 ✗ (entire right subtree must be > 10)
```
...
index=0: Create 8, range (-∞, +∞)
  Left: range (-∞, 8)
    index=1: Create 5, range (-∞, 8)
      Left: range (-∞, 5)
        index=2: Create 1
      Right: range (5, 8)
        index=3: Create 7
...
|-----------|-------|----------|---------------------|
| Preorder | Root → L → R | Copy tree, prefix expression | Easy |
| Inorder | L → Root → R | BST sorting, infix expression | Medium |
| Postorder | L → R → Root | Delete tree, postfix expression | Hard |
...
Master these tree patterns and you'll solve any tree problem recursively or iteratively! 🌲
